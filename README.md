...
Every PNG under `docs/screenshots/` is generated by a single script — never
manually re-shot. After a UI change, regenerate them with:

```bash
venv/bin/python scripts/capture_screenshots.py --strategy blit --window 1280x800
```

The script boots `MainFrame`, switches between every sidebar tab + onboarding
step, and saves a PNG of each. On macOS it can also use the native
`/usr/sbin/screencapture` CLI (`--strategy screencapture`) to better capture
`wx.html2.WebView` contents that the cross-platform `wx.WindowDC.Blit`
fallback sometimes misses. Pass `--no-modals` / `--no-onboarding` to skip
specific groups, or `--tabs today,learn` to capture a subset.

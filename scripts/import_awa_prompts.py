"""Import GRE AWA Issue prompts into the AWAPrompt table."""
...
from models.database import db, init_db, AWAPrompt
...
def is_claim_reason(prompt_text):
    return prompt_text.startswith("Claim:") and "\nReason:" in prompt_text
...
def import_prompts():
    ...
    existing = {
        p.prompt_text[:50]
        for p in AWAPrompt.select(AWAPrompt.prompt_text)
    }
    ...
    for text in PROMPTS:
        ...
        instructions = (
            CLAIM_REASON_INSTRUCTIONS if is_claim_reason(text) else STANDARD_INSTRUCTIONS
        )
        AWAPrompt.create(
            prompt_text=text,
...
if __name__ == "__main__":
    inserted, skipped = import_prompts()
    print(f"Done. Inserted: {inserted}, Skipped (duplicates): {skipped}")
    print(f"Total prompts in pool: {inserted + skipped} processed, "
          f"{len(PROMPTS)} in script.")
